...
@property (nonatomic, strong) LMNLine *tmpline;
@end
...
- (void)updateExclusionPaths
{
    ...
    [text enumerateLinesUsingBlock:^(NSString *textLine, BOOL *stop) {
        ...
        if ([line isKindOfClass:[LMNSpecialLine class]]) {
            ...
            NSLog(@"%@",imageLine.bindingImageView);
...
- (LMNImageView *)insertImage:(UIImage *)image atIndex:(NSInteger)index
{
    ...
    [manager POST:@"https://sm.ms/api/v2/upload" parameters:nil constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
        [formData appendPartWithFormData:data name:@"smfile" ];
        [formData appendPartWithFileData:data name:@"smfile" fileName:@"img" mimeType:@"image/jpg"];
        ...
        if([responseObject[@"code"] isEqualToString:@"success"])
        {
           ...
           imgURL = responseObject[@"data"][@"url"];
           NSLog(@"%@",imgURL);
           [(LMNImageLine *)self.tmpline bindimgURL:imgURL];
    ...
    //以换行开始，因为图片添加之后，就直接从图片的string下一行开始
    ...
    macro_commitUpdating({
        ...
        self.tmpline = newline;
        [(LMNImageLine *)newline bindImageView:imageView];
//        [(LMNImageLine *)newline bindimgURL:imgURL];
